...
int server_port = 9000;              // 리눅스 서버 포트 번호
...
void loop() {
  ...
  Serial.print("보낸 값 : ");
  Serial.println(msg); // 확인용으로 프린트
  client.write(msg); // 입력받은 값을 서버로 전송
  

  if(Serial1.available()){ //서버에게 보낸 값이 다시 잘 돌아왔다면 
    String serverMsg = Serial1.readString();
    Serial.println("받은 값 : ");
