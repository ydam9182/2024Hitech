...
// Wi-Fi 네트워크 정보
char ssid[] = "PYD";        // Wi-Fi 네트워크 이름
char pass[] = "gksksla12@";    // Wi-Fi 비밀번호

// 리눅스 채팅 서버 정보
char server_ip[] = "192.168.1.75";  // 리눅스 서버 IP 주소
...
void loop() {
  ...
  if(Serial1.available()){
    ...
    while(Serial1.available()) {
      c = Serial1.read();
      serverMsg[serv_buf++]= c; //버퍼 저장
    }
    serverMsg[serv_buf] = '\0'; //마지막은 닫아주고

    pToken = strtok(serverMsg, ":"); // 콜론 기점으로 앞에 필요없는값 버리고
    pToken = strtok(NULL, "*"); //*기점으로 으로 실제 메세지 저장하기
